...
To formalize the market regime detection problem, we created a probabilistic generative model that maps the observable return sequence (1958-1998) to a sequence of latent regimes (1998-2025). We opted for using a Hidden Markov Model that uses several structural assumptions that identify a specific pattern of dependencies between the hidden variables and the observed data. 
\subsection*{Assumptions}
\subsubsection*{First-Order Markov Assumption}
Our first assumption comes from one of the definitions of markov chains that states

\[P\left(S_{t}  \mid  S_{1:t-1}\right)=P\left(S_{t} \mid  S_{t-1}\right)\]

This reflects the idea that market volatility rates are persistent and evolve gradually. Individual, isolated one day crashes or jumps are rare. 


\subsubsection*{Conditional Independence of returns given the regime}
Each daily log return \(r_t\) depends only on the current regime:

\[P\left( r_{t} \mid S_{1:t},r_{1:t-1}\right)= P\left( r_{t} \mid S_{t}\right)\]

This assumption captures the idea that once a market’s current state is known, additional past returns do  not provide extra information about the distribution of \(r_t\).



\subsubsection*{Gaussian emission model}
For each regime \(k\), returns follow a Gaussian distribution:
\[r_{t} \mid S_{t} = k  \sim \mathcal{N}\left(\mu_{k},\sigma_{k}^{2}\right)\]

Where \(\mu_{k}\) is the mean daily return and \(\sigma_{k}^{2}\). This aligns with the observation once volatility regimes are accounted for, daily financial returns are approximately Gaussian.\footnote{\cite{classifying-market-regimes}}

\subsubsection*{Stationarity Within Regimes}
The parameters \(\mu_{k}, \sigma_{k}^{2}\) do not change over time within a regime, this approximates the markets as piecewise stationary

\subsubsection*{Dates are not shuffled}

There is no random ordering of the data. We train on the early years and test on the later years to mimic real forecasting. 

\subsection*{Parameters}
Let \( K \) be the number of regimes.
...
This encodes:
\begin{itemize}
    \item regime persistence (diagonal entries)
    \item switching behavior (off-diagonal entries)
    \item expected duration: \(\mathbb{E}[D_k] = \frac{1}{1 - A_{kk}}\)
\end{itemize}
...
For each regime \( k \):
\begin{itemize}
    \item mean return: \( \mu_k \)
    \item variance (volatility): \( \sigma_k^2 \)
\end{itemize}

Financial interpretation:
\begin{itemize}
    \item high-volatility regimes have large \( \sigma_k \)
    \item bull markets have positive \( \mu_k \)
    \item crisis regimes may have negative \( \mu_k \)
\end{itemize}

\subsection*{Structure of dependencies}
\begin{figure}[h]
\centering
\begin{tikzpicture}[
    node distance = 2.0cm,
...
\begin{subfigure}{0.8\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../plots/return_hist.pdf}
...
\textbf{Charlie} \\
I contributed technical components of the project, including data collection and preprocessing, implementing the log-return pipeline, building and debugging the Gaussian HMM, running EM training and model selection, and performing the regime analysis with plots and statistical summaries.  
